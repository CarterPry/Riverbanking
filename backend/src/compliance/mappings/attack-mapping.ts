...
export const attacks: Attack[] = [
  // Enumeration Phase - Run these first to discover attack surface
  {
    id: 'subdomain-enumeration',
    name: 'Subdomain Enumeration',
    description: 'Discover all subdomains for comprehensive testing',
    tsc: ['Security'],
    cc: ['CC6.6', 'CC7.1'],
    tools: [
      { name: 'subdomain-scanner', command: ['subfinder', '-d', '{domain}'] }
    ],
    requiresAuth: false,
    progressive: true,
  ...
  },
  {
    ...
    description: 'Find hidden directories, backup files, and sensitive information',
    tsc: ['Security'],
    cc: ['CC6.1', 'CC6.6'],
    tools: [
      { name: 'directory-scanner', command: ['gobuster', 'dir', '-u', '{target}'] }
    ],
    requiresAuth: false,
    progressive: true,
  ...
  },
  {
    ...
    cc: ['CC6.1', 'CC7.1'],
    tools: [
      { name: 'api-security-scan', command: ['zap-api-scan', '-t', '{target}'] }
    ],
    requiresAuth: false,
    progressive: true,
    evidenceRequired: ['api_endpoints', 'methods', 'parameters'],
    severity: 'medium',
    category: 'CUSTOM'
  },
  {
    id: 'jwt-testing',
    name: 'JWT Token Security Testing',
    description: 'Test JWT tokens for algorithm confusion, weak secrets, and signature vulnerabilities',
    tsc: ['Security', 'Authentication'],
    cc: ['CC6.1', 'CC6.2'],
    tools: [
      { name: 'jwt-scanner', command: ['jwt_tool', '{token}', '-M', 'at'] }
    ],
    requiresAuth: true,
    progressive: false,
...
let attackEmbeddingsData: { [attackId: string]: number[] } = {};
...
try {
  ...
  // Extract attack pattern embeddings
  if (embeddingsJson.attack_patterns) {
    for (const [attackId, data] of Object.entries(embeddingsJson.attack_patterns)) {
      if ((data as any).embedding) {
        attackEmbeddingsData[attackId] = (data as any).embedding;
      }
    }
  }
  console.log(`Loaded ${Object.keys(attackEmbeddingsData).length} precomputed attack embeddings`);
} catch (error) {
  console.warn('Failed to load precomputed embeddings:', error);
}

export const attackEmbeddings: { [attackId: string]: number[] } = attackEmbeddingsData;
